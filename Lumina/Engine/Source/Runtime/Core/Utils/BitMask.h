...
namespace Lumina
{
    ...
    public:
        ...
        inline explicit BitFlags( uint32_t flags ) : BitMask( flags ) {}

        //-------------------------------------------------------------------------

        FORCEINLINE uint32_t Get() const { return BitMask; }
        FORCEINLINE void Set( uint32_t flags ) { BitMask = flags; }
        inline operator uint32_t() const { return BitMask; }

        FORCEINLINE bool HasNoFlagsSet() const { return BitMask == 0; }
        FORCEINLINE bool IsAnyFlagSet() const { return BitMask != 0; }
        ...
        FORCEINLINE bool IsFlagSet( uint8_t flag ) const
        {
            ...
            return ( BitMask & GetFlagMask( flag ) ) > 0;
        }
        ...
        FORCEINLINE BitFlags& operator| ( uint8_t flag )
        {
            Assert( flag < MaxFlags );
            BitMask |= GetFlagMask( flag );
            return *this;
...
namespace Lumina
{
    ...
    public:
        ...
        FORCEINLINE bool IsFlagSet( T flag ) const { return BitFlags::IsFlagSet( (uint8_t) flag ); }
        FORCEINLINE bool IsFlagCleared( T flag ) const { return BitFlags::IsFlagCleared( (uint8_t) flag ); }
        FORCEINLINE void SetFlag( T flag ) { BitFlags::SetFlag( (uint8_t) flag ); }
        FORCEINLINE void SetFlag( T flag, bool value ) { BitFlags::SetFlag( (uint8_t) flag, value ); }
        FORCEINLINE void FlipFlag( T flag ) { BitFlags::FlipFlag( (uint8_t) flag ); }
        FORCEINLINE void ClearFlag( T flag ) { BitFlags::ClearFlag( (uint8_t) flag ); }
