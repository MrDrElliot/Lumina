...
namespace Lumina
{
    ...
    {
    	FShaderCompilationResult compilation_result = { .valid = true };
		...
		for (auto& stage_source : separated_sources) {
			...
			{
				LE_LOG_ERROR("Failed to compile shader");
				compilation_result.valid = false;
			}
