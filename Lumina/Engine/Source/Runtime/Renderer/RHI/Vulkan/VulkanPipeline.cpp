...
namespace Lumina
{
    ...
    void FVulkanPipeline::CreateGraphics()
    {
		...
		if (Specification.input_layout.GetStride())
		{
			...
			for (const auto& element : Specification.input_layout.GetElements())
			{
				...
				{
					location = PreviousLocWidth;
					PreviousLocWidth += (glm::int32)LocationComputeResult;
				}
			...
			VertexInputState.vertexAttributeDescriptionCount = (glm::uint32)VertexInputAttributes.size();
		}
		else
		{
    	...
    	/* Scissor */	
		VkRect2D Scissor = {};
		Scissor.extent = { (glm::uint32)CurrentSwapchainExtent.x, (glm::uint32)CurrentSwapchainExtent.y };
		Scissor.offset = { 0,0 };
		...
		PipelineRendering.colorAttachmentCount = formats.size();
		...
		std::vector<VkPipelineShaderStageCreateInfo> StageInfos = VkShader->GetCreateInfos();
		...
		GraphicsPipelineCreateInfo.stageCount = StageInfos.size();
		...
		VkResult result = vkCreateGraphicsPipelines(Device, VK_NULL_HANDLE, 1, &GraphicsPipelineCreateInfo, nullptr, &Pipeline);
		if(result != VK_SUCCESS)
		{
			LE_LOG_ERROR("Failed to create pipeline \"{0}\".", Specification.debug_name);
			return;
		}

		LE_LOG_TRACE("Pipeline \"{0}\" created successfully", Specification.debug_name);	
    }
    ...
    void FVulkanPipeline::CreateCompute()
    {
    	...
    	std::vector<VkPipelineShaderStageCreateInfo> StageCreateInfo = VkShader->GetCreateInfos();
    	...
    	VkResult result = vkCreateComputePipelines(Device, VK_NULL_HANDLE, 1, &ComputePipelineCreateInfo, nullptr, &Pipeline);
    	if (result != VK_SUCCESS)
    	{
    		LE_LOG_ERROR("Failed to create pipeline \"{0}\".", Specification.debug_name);
    		return;
