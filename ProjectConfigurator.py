...
def create_premake_files(folder, name):
    ...
    template_premake_project_file_path = os.path.join(lumina_engine_directory, "Templates/Project/premake_project_template.txt")
    premake_project_file_path = os.path.join(folder, "Source/premake5.lua")

    with open(template_premake_project_file_path) as premake_file:
        template_content = premake_file.read()

    template_content = template_content.replace("$PROJECT_NAME", name)

    with open(premake_project_file_path, "w") as premake_file:
        premake_file.write(template_content)
...
def create_project_directories(directory, name):
    ...
    content_directory = os.path.join(directory, "Content/Game")
